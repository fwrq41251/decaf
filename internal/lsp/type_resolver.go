...
func (r *typeResolver) resolve(name string) string {
	...
	// 0. If it's already an FQN (contains dots), convert to symbol and check index.
	...
	if baseName == "" || isPrimitive(baseName) {
		return ""
	}
	...
	if r.pkg != "" {
		fqn := r.pkg + "." + baseName
		sym := fqnToSymbol(fqn)
		if syms := r.idx.TypeBySimpleName(baseName); len(syms) > 0 {
			for _, s := range syms {
				if s.Symbol == sym {
					return sym
				}
	...
	javaLangSym := "java/lang/" + baseName + "#"
	if syms := r.idx.TypeBySimpleName(baseName); len(syms) > 0 {
		for _, s := range syms {
